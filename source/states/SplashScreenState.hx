...
class SplashScreenState extends FlxState
{
	...
	function onComplete(Tween:FlxTween):Void
	{
        ...
        {
            var ngSpr:FlxSprite = new FlxSprite(0, FlxG.height * 0.52).loadGraphic(Paths.image('title/newgrounds_logo'));
            add(ngSpr);
            ngSpr.setGraphicSize(290);
            ngSpr.updateHitbox();
            ngSpr.screenCenter();
            ngSpr.antialiasing = true;

            FlxG.sound.play(Paths.sound("recordScratch", "preload"), 0.7);
